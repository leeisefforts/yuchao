...
namespace yuchao.Service
{
    public class ScheduleRecordService : BaseDb , IScheduleRecordService
    {
        ...
        {
            return rdb.AsQueryable().OrderBy("useTime desc").Where(p =>  p.VenueId == venueId && DateTime.Parse(p.UseTime) >= DateTime.Parse(DateTime.Now.ToString("yyyy-MM-dd"))).ToPageList(pageIndex, pageSize);
        }
